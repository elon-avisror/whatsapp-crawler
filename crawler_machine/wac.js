...
const QRfileURL = "http://whatsapp-crawler/assets/QR.png";
...
async function generateMessageEnvelope(groupName, groupCreatedDateString) {
  ...
  if (checkReferenceMessage) {
    ...
    await new Promise((resolve, reject) => {
      ...
      value => {},
      reason => {
        if (DEBUG) console.log("no message flicked", reason);
      }
    );
...
async function getLastMessage(groupName, groupCreatedDateString) {
  ...
  var p = new Promise((resolve, reject) => {
    ...
    value => {},
    reason => {
      if (DEBUG) console.log("last message error", reason);
    }
  );
  await p;
...
async function getValidationLinks() {
  ...
  var req = http.request(options, function (res) {
    ...
    res.on("error", e => {
      console.log("\x1b[33m%s\x1b[0m", "error in get validation link ", e);
    });
  });
  req.end();
}
