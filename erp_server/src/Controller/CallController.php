...
class CallController extends AbstractController
{
    ...
    public function registerCall(Request $request){
        ...
        try{
            ...
            if ($ud_call != 'no_entity_found')  return new JsonResponse(array("message"=>"this call is already registered","msg_ig"=>$msg_id,"msg"=>$data['description']));
            $call = new UdCall();
    ...
    public function registerMessage(Request $request){
        ...
        if(gettype($data['route'] == 'string') && gettype($data['sender_id'] == 'string') && gettype($data['timestamp'] == 'integer') && $data['msg_id'] !=null && $data['msg_id'] !=''){
            $ud_call = $this->getIDByVariable(null,$data['msg_id'],'msgBYMsg_id');
            if ($ud_call != 'no_entity_found')  return new JsonResponse(array("message"=>"this message is already registered","msg_ig"=>$data['msg_id'],"msg"=>$data['msg']));
