...
// 0 - initial scope
// the puppeteer web-browser await until the initial page of whatsapp open, so no need for more help
...
function updateRegularJsonFile(
    ...
    regularJsonFile.msg_id = md5(msg + sender_id + jsonTime);
    regularJsonFile.ts = Date.now();
    regularJsonFile.sender_id = sender_id;
    if (jsonSender !== "")
        regularJsonFile.reference_msg_id = md5(
...
(async () => {
    try {
        ...
        const browserOptions = {
            ...
            userDataDir: "C:\\Users\\ElonAvisror\\Downloads\\Work\\Cambium\\projects\\unidress\\unidress\\unidress_crawler\\browser\\archive" + userDataLocation
        };
        ...
        let elements = $(RIGHT_BAR).toArray();
        ...
        var startCrawler = runCrawlerListener(config);

        startCrawler.forEach(element => {

            // right bar new messages
            if (element.innerText !== "") {
                // open group conversation --> open group description --> wait 5 sec --> quit for next element
                console.log("WORK!");
            }
        });

        console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
        console.log("Listeners are off!");
        console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");

        startCrawler.disconnect();
