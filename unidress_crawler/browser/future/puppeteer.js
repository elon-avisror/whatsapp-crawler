...
function runCrawlerListener(config) {
  let flag = true;
  let noYes = false;
  ...
  try {
    ...
    async function startWhatsApp() {
      return new Promise(async function (resolve, reject) {
        console.log("Starting WhatsApp Crawler");

        var random = Math.floor(Math.random() * userConfig.proxy.length);
        // args.push(userConfig.proxy[random]);
    ...
    let startCrawler = runCrawlerListener(config);
